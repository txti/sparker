...
class CommonNodePruningSpec extends AnyWordSpec with Matchers with SharedSparkContext {
  def blocksToBlockIndex(blocks: RDD[BlockAbstract]): Broadcast[scala.collection.Map[Long, Array[Set[Long]]]] = {
    val m = blocks.map { case block => (block.blockID, block.profiles) }.collect.toMap
    blocks.sparkContext.broadcast(m)
  }
  def blocksToBlocksSize(blocks: RDD[ProfileBlocks]): Broadcast[scala.collection.Map[Long, Long]] = {
    ...
    "calc CBS" in {
      ...
      val actual = Converters.blocksToProfileBlocks(blocks).collect.map { case pb =>
        CommonNodePruning.calcCBS(
          ...
          separatorID = Array(1),
          useEntropy = false,
          blocksEntropies = sc.broadcast(Map.empty[Long, Double]),
          weights = Array.fill[Double](maxID + 1) { 0.0 },
          entropies = Array.ofDim[Double](maxID + 1),
          neighbours = Array.ofDim[Long](maxID + 1),
          firstStep = false
        )
      }
  ...
  "CommonNodePruning.calcWeights" should {
    def weightCalcHelper(blocks: RDD[BlockAbstract], maxID: Long, separatorID: Array[Long], weightType: String): List[List[Double]] = {
      ...
      val profileToNeighbours: Map[Long, (Array[Long], Long, Array[Double])] = {
        Converters.blocksToProfileBlocks(blocks).collect.map { case pb =>
          val neighbours: Map[Long, Long] = Map()
          val weights: Map[Long, Double] = Map()
          (0L to maxID) foreach { id =>
            weights(id) = 0.0
          }
  ...
  "CommonNodePruning.doReset" should {
    "reset the given weights" in {
      ...
      CommonNodePruning.doReset(
        weights,
        neighbours = Array(0L, 1L, 2L).toMap { case i => (i, i) }
        entropies = Array(),
        useEntropy = false,
        neighboursNumber = 3
      )
