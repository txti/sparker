...
object EntityMatching {
  ...
  /**
    ...
    */
  def getComparisons(candidatePairs : RDD[UnweightedEdge], profiles : RDD[(Long, Profile)]) : RDD[(Profile, Array[Long])] ={
    candidatePairs
      .map(p => (p.firstProfileID, Array(p.secondProfileID)))
      .reduceByKey(_++_)
  ...
  /**
    ...
    */
  def getProfilesMap(profiles : RDD[(Long, Profile)] ) : RDD[Map[Long, Profile]] = {
    profiles
      .mapPartitions {
        partitions =>
 ...
 def entityMatchingCB(profiles : RDD[Profile], candidatePairs : RDD[UnweightedEdge], bcstep : Int,
  ...
  : (RDD[WeightedEdge], Long) = {
    ...
    val profilesID = profiles.map(p => (p.id, p))
