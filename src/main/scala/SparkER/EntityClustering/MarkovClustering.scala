...
object MarkovClustering extends EntityClusteringTrait {
  ...
  override def getClusters(
    profiles: RDD[Profile], edges: RDD[WeightedEdge], maxProfileID: Long,
    edgesThreshold: Double, separatorID: Long): RDD[(Long, Set[Long])] = {

    getClusters(profiles, edges, maxProfileID, edgesThreshold, separatorID,
    matrixSimThreshold = 1.0E-5, similarityChecksLimit = 2,
  ...
  def getClusters(profiles: RDD[Profile], edges: RDD[WeightedEdge], maxProfileID: Long, edgesThreshold: Double, separatorID: Long, similarityChecksLimit: Int,
                  matrixSimThreshold: Double, clusterThreshold: Double): RDD[(Long, Set[Long])] = {
