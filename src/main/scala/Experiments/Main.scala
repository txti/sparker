...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    /** Loads two datasets
      */
    val path = "python/datasets/clean/movies/"

    val dataset1 = JSONWrapper.loadProfiles(
      path + "dataset1.json",
      realIDField = "realProfileID",
      sourceId = 1
    )
    val maxIdDataset1 = dataset1.map(_.id).max()

    val dataset2 = JSONWrapper.loadProfiles(
      path + "dataset2.json",
      realIDField = "realProfileID",
      sourceId = 2,
      startIDFrom = maxIdDataset1 + 1
    )
    ...
    val groundtruth = JSONWrapper.loadGroundtruth(
      path + "groundtruth.json",
      firstDatasetAttribute = "id1",
      secondDatasetAttribute = "id2"
    )

    // Converts the id in the groundtruth to the autogenerated ones
    val realIdIds1 = sc.broadcast(
      dataset1
        .map { p =>
          (p.originalID, p.id)
        }
    ...
    val profileBlocksFiltered =
      BlockFiltering.blockFiltering(profileBlocks, 0.8)
    val blocksAfterFiltering =
      Converters.profilesBlockToBlocks(profileBlocksFiltered, separators)

    // Metablocking

    val blockIndexMap =
      blocksAfterFiltering.map(b => (b.blockID, b.profiles)).collectAsMap()
    ...
    val profileBlocksSizeIndex: Broadcast[scala.collection.Map[Long, Int]] =
      sc.broadcast(
        profileBlocksFiltered
          .map(pb => (pb.profileID, pb.blocks.size))
          .collectAsMap()
      )
    ...
    val blocksEntropiesMap: Broadcast[scala.collection.Map[Long, Double]] = {
      if (useEntropy) {
        val blocksEntropies =
          blocks.map(b => (b.blockID, b.entropy)).collectAsMap()
